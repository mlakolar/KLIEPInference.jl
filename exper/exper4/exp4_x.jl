# balanced sample sizes, single-edge for base x, a grid of λ1
...
for t = 1:5
    ...
    θ = spKLIEP(Ψy, Ψx, λ1[t], CD_KLIEP())
    θ = convert(SparseVector, θ)
    supp = union(idx, θ.nzind)
    θ = SparseIterate(θ)
    spKLIEP_refit!(θ, Ψy, Ψx, supp)
    ...
    θhat[1, t] = -KLIEP_debias(idx, θ, ω, Ψy, Ψx)
    σhat[1, t] = KLIEP_var(Ψy, Ψx, θ, ω)
    ...
    θhat[2, t] = -θ[idx]
    σhat[2, t] = KLIEP_var(Ψy[supp,:], Ψx[supp,:], θ[supp], ω[supp])
end
...
@save "/home/bkim6/single/$(graphtype)/exp4_x_rep_$(rep).jld" θhat σhat
